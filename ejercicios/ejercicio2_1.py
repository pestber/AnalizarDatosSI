...
def home():
    ...
    result_incidencias = df_incidencia_mas_tiempo.head(5).apply(lambda x: f"{x['tipo_incidencia']} - {x['count']}", axis=1).str.cat(sep=", ")

    string_final = "Clientes con más incidencias (id - número de incidencias): " + result_clientes + "<br>Incidencias con más tiempo (id - tiempo total): " + result_incidencias
    return string_final
